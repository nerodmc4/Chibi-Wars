...
public class Ferr2DT_PathTerrain : MonoBehaviour, Ferr2D_IPath {
	...
	private void SlicedQuad(List<Vector2> aSegment, int aVert, float aStart, float aEnd, int aCuts, bool aSmoothed, bool aClosed, Ferr2DT_SegmentDescription aDesc, float aStartScale, float aEndScale) {
		...
		for (int i = 0; i < aCuts; i++) {
			...
			if (i != 0) {
				if (!splitMiddle) {
					DMesh.AddFace(v2, p2, p1, v1, false);
				} else {
					DMesh.AddFace(v2, p2, p3, v3, false);
					DMesh.AddFace(v3, p3, p1, v1, false);
				}
			}
	...
	private void AddCap    (List<Vector2> aSegment, Ferr2DT_SegmentDescription aDesc, float aDir, float aScale, bool aSmooth) {
        ...
        if (splitMiddle && aDir < 0) {
            DMesh.AddFace(v1,  v2,  v25, v15, false);
            DMesh.AddFace(v15, v25, v3,  v4, false );
        } else if (splitMiddle && aDir >= 0) {
            DMesh.AddFace(v2,  v1,  v15, v25, false);
            DMesh.AddFace(v25, v15, v4,  v3, false);
        } else if (aDir < 0){
            DMesh.AddFace(v1, v2, v3, v4, false);
        } else {
            DMesh.AddFace(v2, v1, v4, v3, false);
        }
	}
	private void AddFill   (bool aSkirt) 
	{
		...
		if (isDestructible && fill != Ferr2DT_FillMode.InvertedClosed)
		{
			...
			Ferr2D_Triangulator.GetVerticesAndIndices(fillVerts, out newFillVerts, out indices, chunkSizeX, chunkSizeY, chunkJitterX, chunkJitterY);
			fillVerts = newFillVerts;
		}
		else
		{
			indices = Ferr2D_Triangulator.GetIndices(ref fillVerts, true, fill == Ferr2DT_FillMode.InvertedClosed);
		}
