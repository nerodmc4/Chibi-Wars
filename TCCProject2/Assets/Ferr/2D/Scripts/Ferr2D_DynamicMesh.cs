...
public class Ferr2D_DynamicMesh
{
    ...
    public void  ExtrudeZ              (float    aDist, bool aInverted)
    {
        ...
        {
            if (aInverted) AddFace(i, i + count,i + count + 1, i+1, false);
            else           AddFace(i + 1, i + count + 1, i + count, i, false);
        }

        if (aInverted) AddFace(count - 1, (count - 1) + count, count, 0, false);
        else           AddFace(0, count, (count - 1) + count, count - 1, false);
    }
    /// <summary>
