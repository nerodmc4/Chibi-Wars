...
public class EQ_TestParticles : MonoBehaviour
{
	...
	void OnGUI ()
	{
		...
		GUI.Window(2, new Rect(10, Screen.height-65, 290, 60), DemoSceneWindow, "Demo Scenes");
		
		// Show Information GUI window
		GUI.Window(3, new Rect((Screen.width-360), Screen.height-85, 350, 80), ParticleInformationWindow, "Information");
	}
	...
	void ParticleInformationWindow(int id)
	{
		if(m_CurrentParticleIndex>=0)
		{
			GUI.Label(new Rect(12, 25, 350, 20), "Up / Down: Type ("+(m_CurrentCategoryIndex+1)+" of "+m_CategoryList.Length+" "+m_CurrentCategoryName+")");
			GUI.Label(new Rect(12, 50, 350, 20), "Left / Right: Particle ("+(m_CurrentParticleIndex+1) + " of " + m_CurrentCategoryChildCount +" "+ m_CurrentParticleName+")");
		}
	}
